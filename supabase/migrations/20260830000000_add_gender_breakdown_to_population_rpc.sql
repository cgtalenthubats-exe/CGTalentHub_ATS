-- Add gender breakdown to the Candidate Funnel dashboard tab's population RPC,
-- matching the same pattern already used for nationality: a "by_gender" list
-- of {name, count} plus a "gender_unknown_count" so the UI can show
-- "By Gender (N Unknown) · Total: X" the same way it does for Nationality.
--
-- Only "Male"/"Female" count as known — every other raw value seen in
-- Candidate Profile.gender (N/A, Unknown, Not Specified, None, blank casing
-- variants, etc.) is bucketed into unknown rather than enumerated, since new
-- junk values can appear at any time and shouldn't silently stop counting as
-- "unknown".
...
BEGIN
    ...
    SELECT json_build_object(
        ...
        'by_gender', COALESCE((
            SELECT json_agg(row_data) FROM (
                SELECT json_build_object('name', gender, 'count', COUNT(DISTINCT candidate_id)) AS row_data
                FROM latest_only
                WHERE gender IN ('Male', 'Female')
                GROUP BY gender ORDER BY COUNT(DISTINCT candidate_id) DESC
            ) t
        ), '[]'::json),
